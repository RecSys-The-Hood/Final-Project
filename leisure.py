...
def filter_geojson_by_coordinates(geojson_file, city_name, city_coordinate_ranges):
    ...
    for feature in geojson_data['features']:
        ...
        if (lat_range[0] <= latitude <= lat_range[1]) and (lon_range[0] <= longitude <= lon_range[1]):
            ...
            leisure = feature.get('properties', {}).get('leisure')
            shop = feature.get('properties', {}).get('shop')
            ...
            data = {
                ...
                'longitude': longitude,
                'city': city_name,
                'type': 'park' if leisure == 'park' else 'mall' if shop == 'mall' else None,
                'name': name
            }
...
df.to_csv('combined_leisure.csv', index=False)
